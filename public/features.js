...
function renderPurchasePlan(result){
  ...
  const update=()=>{
    ...
    if(!required){
      if(!valid.length){$('priceAnswer').innerHTML='<b>$0.00 estimated purchase</b><span>Your current plan shows no additional units needed. Enter prices above to compare package value.</span>';return;}
      const best=valid.map(x=>({...x,units:Number(x.units),price:Number(x.price),unitPrice:Number(x.price)/Number(x.units)})).sort((a,b)=>a.unitPrice-b.unitPrice||a.price-b.price)[0];
      $('priceAnswer').innerHTML=`<b>$0.00 estimated purchase</b><span>No additional units needed. Cheapest entered option: ${esc(best.label||'Option')} — ${best.units} unit${best.units===1?'':'s'} for $${best.price.toFixed(2)} ($${best.unitPrice.toFixed(2)}/unit).</span>`;return;
    }
