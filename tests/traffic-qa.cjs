...
(async()=>{
 ...
 for(const [slug,title,r1] of tools){
  ...
  assert((await page.locator('link[rel="canonical"]').getAttribute('href'))===base+slug,slug+' canonical mismatch');
 ...
 console.log('TRAFFIC QA PASSED: 5 calculators, 2 deck-spacing guides, homepage hub, mobile layout.');
 await browser.close();
